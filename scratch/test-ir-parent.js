...
async function testIRRemotes(infraredId) {
  try {
    const accessToken = await getAccessToken();
    const timestamp = Date.now().toString();
    const path = `/v2.0/infrareds/${infraredId}/remotes`;
    ...
    const data = await res.json();
    console.log(`\n=== REMOTES FOR INFRARED: ${infraredId} ===`);
...
async function run() {
  await testIRRemotes('a3ed7d17d678229073rbws');
}
