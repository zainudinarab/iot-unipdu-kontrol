...
async function tryCommand(code, value) {
  try {
    const accessToken = await getAccessToken();
    const timestamp = Date.now().toString();
    const path = `/v1.0/devices/${acId}/commands`;
    ...
    const data = await res.json();
    if (data.success || data.msg !== 'command or value not support') {
      console.log(`Testing { code: "${code}", value: ${JSON.stringify(value)} } -> Success: ${data.success}, Msg: ${data.msg || 'None'}`);
    }
  } catch (error) {
    // Ignore fetch errors
  }
...
async function run() {
  const keys = ['speed', 'fan_speed', 'wind_speed', 'FanSpeed', 'WindSpeed', 'Fan', 'Wind', 'Speed'];
  const values = [0, 1, 2, 3, 'low', 'mid', 'high', 'auto'];
  
  console.log('Testing alternative fan keys...');
  for (const key of keys) {
    for (const val of values) {
      await tryCommand(key, val);
    }
  }
