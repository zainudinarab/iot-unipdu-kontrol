...
async function testMode(deviceId) {
  try {
    const accessToken = await getAccessToken();
    const timestamp = Date.now().toString();
    const path = `/v1.0/devices/${deviceId}/commands`;
    const commands = [{ code: 'Mode', value: 'cool' }];
    const bodyStr = JSON.stringify({ commands });
    const signature = getSignature('POST', path, accessToken, timestamp, bodyStr);

    console.log(`Sending mode command to ${deviceId}:`, JSON.stringify(commands));
    const res = await fetch(`${endpoint}${path}`, {
      method: 'POST',
      headers: {
        'client_id': clientId,
...
testMode('a352e2e4b0b6ff61b9rf4k');
