...
async function sendCommand(deviceId, commands) {
  try {
    ...
    console.log(`Sending native command to ${deviceId}:`, JSON.stringify(commands));
    const res = await fetch(`${endpoint}${path}`, {
      method: 'POST',
      headers: {
        'client_id': clientId,
...
async function run() {
  console.log('--- TEST: Sending Native switch=false ---');
  await sendCommand('a352e2e4b0b6ff61b9rf4k', [{ code: 'switch', value: false }]);
}
