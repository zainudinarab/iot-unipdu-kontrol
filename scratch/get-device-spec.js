...
async function getDeviceSpecs(deviceId) {
  try {
    const accessToken = await getAccessToken();
    const timestamp = Date.now().toString();
    
    // Check specifications endpoint
    const path = `/v1.0/devices/${deviceId}/specifications`;
    const signature = getSignature('GET', path, accessToken, timestamp);

    console.log(`Fetching specifications for ${deviceId}...`);
    const res = await fetch(`${endpoint}${path}`, {
      method: 'GET',
      headers: {
        'client_id': clientId,
    ...
    const data = await res.json();
    console.log(`Specifications Result:`, JSON.stringify(data, null, 2));

    // Also check status/functions
    const functionsPath = `/v1.0/devices/${deviceId}/functions`;
    const sigFunctions = getSignature('GET', functionsPath, accessToken, timestamp);
    const resFunc = await fetch(`${endpoint}${functionsPath}`, {
      method: 'GET',
      headers: {
        'client_id': clientId,
        'access_token': accessToken,
    ...
    console.log(`Functions Result:`, JSON.stringify(dataFunc, null, 2));
...
getDeviceSpecs('a352e2e4b0b6ff61b9rf4k');
